...
export function activate(transformation = ID) {
	...
	// ? Question: about spacing ; Does one want to do it like 'RESULT.submodules[thing] = value', or as a part of this definition?
	...
	const RESULT = {
		// * each 'submodule' is just an elementary object, where all this other stuff is also present, but 'grouped' under this one same property;
		submodules: {
			infinite: {
						...
						static: {
							...
							fromArray(arr, leftovers = {}) {
								RESULT.ensureProperties(leftovers, {
									...
									comparison:
										this.this.template.icclass.template.comparison
								})
									...
									A.this = {
										...
										move(
											...
											comparison = this.this.this.this.class
												.template.class.template.icclass.template
												.comparison,
											each = (x) => x.next(),
											stop = (x) =>
											...
											return {
												...
												get() {
													...
													while (
														...
														this.object().next()
													const returned =
														this.object().currindex
													this.object().currindex = index
													return returned
												},
												set(value, leftovers = {}) {
													RESULT.ensureProperties(leftovers, {
														range: this.object().this.class
										...
										*[Symbol.iterator](leftovers = {}) {
										...
										convert(
											...
											RESULT.ensureProperties(leftovers, {
												fast: false,
												range: templates[0].icclass.template
													.range,
												comparison:
													templates[0].icclass.template
														.comparison
											})
											const newArr = RESULT.GeneralArray(
												templates[0]
											)(templates[1])()
											this.this.this.loop()._full(
												newArr.pushbackLoop({
										...
										projectFit(array, index, leftovers = {}) {
											...
											this.this.this.loop()._full(
												...
												(x) =>
													...
													array.this.class.template.isEnd(
														array
													),
												(t) =>
													t.object().go(index, leftovers.range)
											)
											array.currindex = ind
										},
										...
										shiftBackward(times, leftovers = {}) {
											...
											this.this.this = this.this.this.slice(
												times.map(
													this.this.this.this.template.class
														.template.icclass
												),
												undefined,
												leftovers
				...
				// TODO: old code of the 'depthOrder' commented out; Pray re-visit later...
				// * _ TODO: implement -- depthOrder([[[0], [1], 2], 3, [[4, [5]]]]) := lastIndexArray([1,2,3,4,5])
				...
				// TODO: pray create an actual InfiniteArray implementation [not that of UnlimitedArray];

				// TODO: implement the actual InfiniteString [truly infinite] + implement the actual UnlimitedString [arbitrarily-lengthed];
				// TODO: implement the TrueInteger class; Same for TrueRatio (pair of integers) + TrueSum/TrueSequence [for an InfiniteSum of TrueRatio-s]; Let they be all highly compatible with InfiniteCounter;
			}
		},
		aliases: {
		...
		methods: {
			 ...
			 */
			repeatedOperation: function (objects = [], operator, table = defaultTable) {
			 ...
			 */
			standardDeviation: function (row = [], isPopulation = true, indexes = []) {
				return floor(
		...
		// ? QUESTION: merge the 'constants' with 'globals'? This'd
		constants: {
	...
	// 3 [code update, minor bugfix, tuning]. Look at the 'PointerArray' code;
